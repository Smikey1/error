...
def create_blog(request: BlogModel, db: Session = db_instance):
    new_blog = db_schemas.BlogSchema(
        title=request.title,
        body=request.body,
...
@router.get(
    "/{id}", status_code=status.HTTP_200_OK, response_model=ShowResponseBlogModel
)
# @router.get("/{id}",status_code=status.HTTP_200_OK)
def get_blog_by_Id(id: int, db: Session = db_instance):
    single_blog = (
        db.query(db_schemas.BlogSchema).filter(db_schemas.BlogSchema.id == id).first()
    )
    if not single_blog:
        # response.status_code = status.HTTP_404_NOT_FOUND
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=failure("Blog not found")
        )
...
def update_blog_by_Id(id: int, request: BlogModel, db: Session = db_instance):
    blog = db.query(db_schemas.BlogSchema).filter(
        db_schemas.BlogSchema.id == request.title
    )
    if not blog.first():
